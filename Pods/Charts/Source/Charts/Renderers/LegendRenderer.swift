...
open class LegendRenderer: Renderer
{
        ...
        case .vertical:
            ...
            for i in 0 ..< entries.count
            {
                ...
                if e.label != nil
                {
                    ...
                    if direction == .rightToLeft
                    {
                        posX -= (e.label as NSString!).size(attributes: [(NSFontAttributeName as NSString) as String: labelFont]).width
                    }
    ...
    {
        ChartUtils.drawText(context: context, text: label, point: CGPoint(x: x, y: y), align: .left, attributes: [NSFontAttributeName as NSString: font, NSForegroundColorAttributeName as NSString: textColor])
    }
}
