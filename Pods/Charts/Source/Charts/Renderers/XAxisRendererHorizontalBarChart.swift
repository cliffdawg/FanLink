...
open class XAxisRendererHorizontalBarChart: XAxisRenderer
{
    ...
    open override func computeSize()
    {
        ...
        let labelSize = longest.size(attributes: [(NSFontAttributeName as NSString) as String: xAxis.labelFont])
            ...
            if viewPortHandler.isInBoundsY(position.y)
            {
                if let label = xAxis.valueFormatter?.stringForValue(xAxis.entries[i], axis: xAxis)
                {
                    drawLabel(
                        ...
                        x: pos,
                        y: position.y,
                        attributes: [NSFontAttributeName as NSString: labelFont, NSForegroundColorAttributeName as NSString: labelTextColor],
                        anchor: anchor,
            ...
            if l.drawLabelEnabled && label.characters.count > 0
            {
                ...
                if l.labelPosition == .rightTop
                {
                    ChartUtils.drawText(context: context,
                        ...
                        align: .right,
                        attributes: [NSFontAttributeName as NSString: l.valueFont, NSForegroundColorAttributeName as NSString: l.valueTextColor])
                }
