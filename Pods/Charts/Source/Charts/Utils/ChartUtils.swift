...
open class ChartUtils
{
    ...
    open class func drawText(context: CGContext, text: String, point: CGPoint, align: NSTextAlignment, attributes: [NSAttributedStringKey : Any]?)
    {
        ...
        if align == .center
        {
            point.x -= text.size(attributes: attributes as! [String : Any]).width / 2.0
        }
        else if align == .right
        {
            point.x -= text.size(attributes: attributes as! [String : Any] as! [String : Any]).width
        }
        
        NSUIGraphicsPushContext(context)
        
        (text as NSString).draw(at: point, withAttributes: attributes as! [String : Any])
    ...
    open class func drawText(context: CGContext, text: String, point: CGPoint, attributes: [NSAttributedStringKey : Any]?, anchor: CGPoint, angleRadians: CGFloat)
    {
        ...
        if angleRadians != 0.0
        {
            let size = text.size(attributes: attributes as! [String : Any])
            ...
            (text as NSString).draw(at: drawOffset, withAttributes: attributes as! [String : Any])
    ...
    internal class func drawMultilineText(context: CGContext, text: String, knownTextSize: CGSize, point: CGPoint, attributes: [NSAttributedStringKey : Any]?, constrainedToSize: CGSize, anchor: CGPoint, angleRadians: CGFloat)
    {
        ...
        if angleRadians != 0.0
        {
            ...
            (text as NSString).draw(with: rect, options: .usesLineFragmentOrigin, attributes: attributes as! [String : Any], context: nil)
    ...
    {
        let rect = text.boundingRect(with: constrainedToSize, options: .usesLineFragmentOrigin, attributes: attributes as! [String : Any], context: nil)
